 ...
 * @author alfar
 */
public class EstudiosMedicosCXPPagosController implements Initializable {
    ...
    private void accionRealizarPago(ActionEvent event) throws SQLException {
        ...
        pagocuentaporpagar.setIdComprasInternasP(estudio.getId());
        pagocuentaporpagar.setFechaPago(sqlDate);
        pagocuentaporpagar.setId_forma_pago(estudio.getFormapagoint());
        ...
        double saldoci = Double.parseDouble(df.format((estudio.getSaldo_saldar() - Double.parseDouble(txfMontoPago.getText()))));
        double montopagado = Double.parseDouble(df.format((estudio.getMonto_abonado()+ Double.parseDouble(txfMontoPago.getText()))));
        estudio.setMonto_abonado(montopagado);
        estudio.setSaldo_saldar(saldoci);

        if (saldoci == 0.0) {
        
            estudio.setEstatusPagoEstudio(1);
        }
        estudio.setUsuarioModificacion(VPMedicaPlaza.userSystem);
    ...
    public void recibirDatosPago(EstudioMedico estudioMedico) {
        DecimalFormat df = new DecimalFormat("0.00");
        estudio = estudioMedico;
        Laboratorio laboratorio = estudioMedico.getLaboratorio();
        lblFolioCompra.setText(""+estudioMedico.getIdFolio());
        lblFolioPedido.setText(""+estudioMedico.getIdFolio());
        lblRazonSocial.setText(laboratorio.getRazonSocial());
        lblRfc.setText(laboratorio.getRfc());

        if (estudio.getEstatusPagoEstudio()== 1) {
        ...
        dtpFechaCompra.setValue(estudioMedico.getFechaPedido().toLocalDate());
        lblMontoCompra.setText("$" + df.format(estudioMedico.getPrecio_con_iva()));
        lblTotalAbono.setText("$" + df.format(estudioMedico.getMonto_abonado()));
        lblSaldo.setText("$" + df.format(estudioMedico.getSaldo_saldar()));

        pagoscuentasporpagardao = new PagosCuentasPorPagarDAO(conexion.conectar2());
        try {
            filas = pagoscuentasporpagardao.obtenerCantidadFilasPorIdOrdenCompra(estudio.getEstatusEstudio());
    ...
    private void llenarTabla() throws SQLException {
        pagoscuentasporpagardao = new PagosCuentasPorPagarDAO(conexion.conectar2());
        pagoscuentasporpagar.addAll(pagoscuentasporpagardao.obtenerTodosLosPagosPorIdOrdenCompra(estudio.getId()));
